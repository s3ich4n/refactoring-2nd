...
def test_regular_booking_base_price(show_with_talkback, weekday, weekend):
    # 주중 예약
    weekday_booking = Booking(show_with_talkback, weekday)
    assert weekday_booking.base_price() == 100

    # 주말(성수기) 예약
    weekend_booking = Booking(show_with_talkback, weekend)
...
def test_regular_booking_has_talkback(
    ...
    # 주중 예약, 톡백 없음
    weekday_booking_no_talkback = Booking(show_without_talkback, weekday)
...
def test_premium_booking_base_price(
    ...
    # 주중 프리미엄 예약
    weekday_premium = PremiumBooking(show_with_talkback, weekday, extras_with_dinner)
    assert weekday_premium.base_price() == 150  # 100 + 50 프리미엄 요금

    # 주말(성수기) 프리미엄 예약
    weekend_premium = PremiumBooking(show_with_talkback, weekend, extras_with_dinner)
...
def test_premium_booking_has_talkback(
    ...
    # 주중 프리미엄 예약, 톡백 없음
    weekday_premium_no_talkback = PremiumBooking(
        show_without_talkback, weekday, extras_with_dinner
    )
