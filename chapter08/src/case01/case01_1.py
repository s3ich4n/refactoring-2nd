...
def track_summary(points):
    def calculate_time():
        ...
        return total_distance(points) * 10

    total_time = calculate_time()
    pace = total_time / 60 / total_distance(points) if total_distance(points) > 0 else 0
    return {"time": total_time, "distance": total_distance(points), "pace": pace}
